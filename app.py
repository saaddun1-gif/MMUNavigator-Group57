...
def find_best_match_by_keywords(user_question: str, knowledge_base: dict) -> tuple | None:
    ...
    # Check for question type FIRST (this solves your problem)
    ...
    for q in knowledge_base.get('questions', []):
        # Skip if intent doesn't match the question type
        ...
        # Boost for exact keyword matches in question
        ...
        if score > best_score and score >= 0.2:  # Lower threshold since we already filtered by intent
            best_score = score
            best_match = q
            best_question = q['question']
    
    # If we found a match with intent, return it
    if best_match:
        print(f"🔍 Matched '{user_question}' with {best_score*100:.0f}% confidence → {best_question}")
        return best_question, best_match['answer']
    
    # FALLBACK: If no intent match, try without intent filtering (for questions without intent field or greetings)
    print(f"⚠️ No intent match found, trying fallback matching...")
    for q in knowledge_base.get('questions', []):
        # Skip if this is a special case that should only be matched by intent
...
def debug_match():
    """Debug endpoint to see what's matching"""
    try:
        data = request.json
        user_message = data.get('message', '')
...
if __name__ == '__main__':
    ...
    print(f"📍 Debug endpoint: http://127.0.0.1:5000/debug/match")
